...
def evaluate_TfidfReader(dataset_path):
    with open(dataset_path) as f:
        dataset = json.load(f)['data']
    f1 = exact_match = total = exact_sentence = inclusion = random = 0
    for article in dataset:
...
# def evaluate_RandomReader(dataset_path):
#     reader = RandomReader()
#     evaluate_reader(dataset_path, reader)


parser = argparse.ArgumentParser()
parser.add_argument('-i', '--input', help='Location Of your input File ', required=True)
